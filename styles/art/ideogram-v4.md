# Ideogram v4 Prompting Rules

Conventions for the `ideogram-v4` art generator profile (`backend: "comfyui"` in
`config/system.json` → `art.generators`). Unlike the tag-based SD 1.5 workflow, Ideogram-class
models consume a structured JSON caption produced by a captioning pass ("splatter planning")
before the image render itself: a natural-language idea goes in, a JSON object describing
composition, elements, and background comes out, and that JSON drives the render. The block
below is the captioning system prompt in full, followed by one worked user turn, preserved
verbatim so it can be copied directly into a captioning step.
...
You cannot split these by region. `sky upper-left portion`, `sky behind the tower`, `sky upper two-thirds` are the SAME component — describe in `background` once. Same for crowd, ground, horizon.
...
Some objects are simultaneously part of the shell AND focal elements that define the room's identity: a chalkboard covering the back wall of a classroom, a fireplace built into a living-room wall, a large mounted TV, a stage proscenium, a built-in reception counter, a built-in bookshelf, a large fixed service desk, a fixed sign/banner.
...
Silent / ambiguous → photograph (default). The subject's reality status does NOT override this default — knights, dragons, aliens, robots in a photograph are valid; the brief must explicitly ASK for illustration / painting / render to get one.
